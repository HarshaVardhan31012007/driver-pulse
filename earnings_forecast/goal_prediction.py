...
class GoalPredictor:
   ...
   def _predict_single_driver_goal(self, driver_data: Dict) -> Dict:
    ...
    def _generate_recommendations(self, status, progress_percentage, driver_data):
        # Fix status type
        ...
        if status == GoalStatus.GOAL_ALREADY_ACHIEVED:
            ...
            recommendations.append("Consider taking a break or earn bonus rewards.")
    
        elif status == GoalStatus.GOAL_ON_TRACK:
            recommendations.append("✅ You're on track!")
            recommendations.append("Maintain your current pace.")
    
        elif status == GoalStatus.GOAL_AT_RISK:
            recommendations.append("⚠️ You're at risk.")
            recommendations.append("Drive during peak hours.")
            if progress_percentage < 50:
                recommendations.append("Focus on longer trips or surge areas.")
    
        elif status == GoalStatus.GOAL_LIKELY_MISSED:
            recommendations.append("❌ Likely to miss goal.")
            recommendations.append("Increase driving time.")
            recommendations.append("Consider adjusting goal.")
    
        elif status == GoalStatus.INSUFFICIENT_DATA:
            recommendations.append("📊 Keep driving for better predictions.")
    
        # Velocity logic
        current_velocity = driver_data.get('current_velocity', 0) or 0
        avg_velocity = driver_data.get('avg_earnings_per_hour', 0) or 0
    
        if avg_velocity > 0:
            if current_velocity < avg_velocity * 0.8:
                recommendations.append("You're below average pace.")
            elif current_velocity > avg_velocity * 1.2:
                recommendations.append("Great pace! Above average.")
    
        # Fallback
        if not recommendations:
            recommendations.append("Keep driving to improve performance.")
